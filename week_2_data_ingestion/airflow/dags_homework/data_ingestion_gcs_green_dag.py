...
csv_file_name = '/green_tripdata_{{dag_run.logical_date.strftime("%Y-%m")}}.csv'
...
# Note, some csv files has ehail_fee with decimals. This causes the parket file include its own schema, and bigquery creates a field type of integer,
# however, when querying, it fails. So let's use float64 for this field. When using later on the tables in dbt, we can cast  in the queries if needed
# Note that specifying the schema when creating external table from parquet format or regular table, will fail (actually no conversion is take in place)
...
with DAG(
    dag_id="data_ingestion_gcs_green_dag_homework",
    schedule_interval="0 6 2 * *",
    default_args=default_args,
    catchup=True,
    max_active_runs=3,
    tags=['dtc-gcp-green-homework'],
) as dag:
